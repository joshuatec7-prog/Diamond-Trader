#!/usr/bin/env python3
"""
Diamond Trader LONG Min-Profit Shadow Lab v1.0

Doel
----
Volgt vanaf de nulmeting ieder NIEUW geldig Diamond Trader LONG-signaal
voor de normale LONG-symbolen uit config.yaml en vergelijkt drie identieke
virtuele posities:

1. CURRENT_100 : minimum netto winst €1,00
2. TEST_050    : minimum netto winst €0,50
3. TEST_025    : minimum netto winst €0,25

ALLE andere regels blijven exact gelijk:
- zelfde LONG-signaal
- zelfde entrymoment
...
- uitsluitend eigen Min-Profit Shadow-bestanden in /var/data

Gebruik
-------
python3 long_min_profit_shadow_lab.py --self-test
python3 long_min_profit_shadow_lab.py --update
python3 long_min_profit_shadow_lab.py --status
...
VERSION = "1.0"
MODE = "READ_ONLY_LONG_MIN_PROFIT_SHADOW"
...
BASELINE_FILE = DATA_DIR / "diamond_long_min_profit_shadow_baseline.json"
STATE_FILE = DATA_DIR / "diamond_long_min_profit_shadow_state.json"
REPORT_FILE = DATA_DIR / "diamond_long_min_profit_shadow_report.json"
TRADES_FILE = DATA_DIR / "diamond_long_min_profit_shadow_trades.csv"
...
def public_exchange() -> ccxt.Exchange:
    # Expliciet GEEN apiKey en GEEN secret.
...
def ingest_signals(
    ...
    for symbol in settings["symbols"]:
        ...
        for idx in range(
            ...
            # Alleen NIEUWE signalen na de nulmeting.
            if (
                candle_close_ms
                <= baseline_ms
            ):
            ...
            # Alle varianten krijgen exact hetzelfde entrymoment.
            entry_target_ms = (
                candle_close_ms
            )
...
def try_fill_pending(
    ...
    # Alleen vullen als de bedoelde candle al gestart is.
    if (
        candle_ts
        > int(
            time.time()
...
def evaluate_open(
    ...
    for _, candle in candles.iterrows():
        ...
        # Zelfde Diamond Trader exitvolgorde.
        # Alleen min_exit verschilt per variant.
        if (
            profit_pct
            >= settings[
                "profit_trigger_pct"
        ...
        if reason is not None:
            ...
            # Stop-loss en hard-stop altijd toegestaan.
            # Andere exits alleen wanneer minimaal de variantgrens
            # NETTO na beide fees overblijft.
            if (
                reason
                not in {
                    "stop_loss",
...
def prepare_frames(
    ...
    # Genoeg terug voor indicatoren en maximaal 48h virtuele posities.
    since_ms = int(
        (
            baseline_dt.timestamp()
            - 36 * 60 * 60
...
def build_report(
    ...
    return {
        ...
        "rules": {
            "entry": (
                "alle varianten exact hetzelfde entrymoment"
            ),
            "variant_difference_only": (
                "minimum netto winst voor winstexit/trailing"
            ),
            "CURRENT_100": (
                "minimum netto winst €1.00"
            ),
            "TEST_050": (
                "minimum netto winst €0.50"
            ),
            "TEST_025": (
                "minimum netto winst €0.25"
            ),
            "max_hold_minutes": (
                MAX_HOLD_MINUTES
            ),
...
def print_report(
    ...
    print(
        " DIAMOND TRADER LONG MIN-PROFIT SHADOW LAB"
    )
    print(
        "=" * 76
    )
    ...
    print(
        f"Voortgang              : {to_float(progress.get('progress_pct'), 0.0):.1f}%"
    )
    print()
    ...
    for variant in VARIANTS:
        ...
        print(
            f"{variant:12s} "
            f"min=€{to_float(row.get('min_profit_eur'), VARIANTS[variant]):.2f} "
    ...
    print(
        "Config gewijzigd        : NEE"
    )
    print(
        "Transacties gewijzigd   : NEE"
    )
...
def run_update() -> Dict[str, Any]:
    ...
    if (
        ...
        raise ValueError(
            "LONG Min-Profit Shadow verwacht timeframe 15m"
        )
...
def self_test() -> None:
    ...
    for variant, min_profit in VARIANTS.items():
        position = {
            **base_position,
    ...
    assert (
        min_prices["CURRENT_100"]
        > min_prices["TEST_050"]
        > min_prices["TEST_025"]
    )

    assert (
        VARIANTS["CURRENT_100"]
        == 1.00
    )
    assert (
        VARIANTS["TEST_050"]
    ...
    assert (
        SAFETY["orders_possible"]
        is False
    )
    assert (
        SAFETY["private_exchange_calls"]
        is False
    )
    assert (
        SAFETY["api_keys_loaded"]
        is False
    )
    assert (
        SAFETY["config_write"]
    ...
    assert (
        SAFETY["bot_state_write"]
        is False
    )

    print(
        "LONG_MIN_PROFIT_SHADOW_SELF_TEST_OK"
    )
    print(
        "Varianten: €1.00 -> €0.50 -> €0.25"
    )
    print(
...
def main() -> None:
    ...
    if args.status:
        ...
        if not report:
            print(
                "Nog geen LONG Min-Profit Shadow rapport. "
    ...
    if args.no_print:
        ...
        print(
            "LONG Min-Profit Shadow bijgewerkt | "
            f"signals={progress.get('signals_detected', 0)}/"
            f"{TARGET_SIGNALS} | "
            f"EUR1_closed="
            f"{(variants.get('CURRENT_100') or {}).get('closed', 0)} | "
            f"EUR050_closed="
            f"{(variants.get('TEST_050') or {}).get('closed', 0)} | "
            f"EUR025_closed="
            f"{(variants.get('TEST_025') or {}).get('closed', 0)}"
        )
