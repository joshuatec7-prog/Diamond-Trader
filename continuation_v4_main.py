...
logger = logging.getLogger('cryptobot_cleanroom_continuation_v4')
...
def continuation_db_path(primary_path: str) -> str:
    ...
    return str(p.with_name(f'{stem}_continuation_v4{suffix}'))
...
def build_trader(s: Settings, db: Storage) -> RunnerProfitProtectPaperTrader:
    return RunnerProfitProtectPaperTrader(
        s,
        db,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description='CryptoBot Clean-Room Strategy C v4 - continuation runner PAPER'
    )
    ...
    try:
        if args.status:
            print('=== STRATEGY C v4 | PULLBACK CONTINUATION | RUNNER | GEEN HARDE TP ===')
        ...
        logger.info(
            'gestart | STRATEGY C v4 RUNNER | PAPER ONLY | interval=%s | universe=%s | '
            'max_open=%s | stop=%.2f%% | hard_take=UIT | protect=%.2f%%->€%.2f | '
            'trail=%.2f%% | db=%s',
        ...
        while True:
            ...
            if ok == len(markets) and failed == 0:
                ...
                continuation_db.set_data_health(
                    'READY', f'volledige continuation-v4-runner-cyclus ok={ok}'
                )
            elif ok > 0:
                consecutive_total_failures = 0
                continuation_db.set_data_health(
                    'PARTIAL',
                    f'continuation-v4-runner-cyclus ok={ok} failed={failed}; {last_error}',
                )
            else:
                consecutive_total_failures += 1
                continuation_db.set_data_health(
                    'DEGRADED',
                    last_error or f'alle {failed} continuation-v4-marktcycli mislukt',
                )

            if consecutive_total_failures >= continuation_s.max_consecutive_failed_cycles:
                logger.error(
                    'continuation-v4-marktdata volledig onbereikbaar gedurende %s cycli',
