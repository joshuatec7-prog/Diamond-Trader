#!/usr/bin/env python3
"""
Diamond Trader - LONG Shadow latest-500 updater v1.0

Gebruik:
  python3 apply_long_shadow_latest500_v1_0.py --check
  python3 apply_long_shadow_latest500_v1_0.py --apply
"""
...
def validate(path: Path, text: str) -> list[str]:
    ...
    if text.count(old_block) != 1:
        problems.append(
            f"{path.name}: since-blok verwacht 1x, gevonden {text.count(old_block)}x"
        )

    if text.count(OLD_FETCH) != 1:
        problems.append(
            f"{path.name}: fetch-anker verwacht 1x, gevonden {text.count(OLD_FETCH)}x"
        )
...
def do_check() -> int:
    print("=== LONG SHADOW LATEST-500 CHECK V1.0 ===")
    failed = False
...
def do_apply() -> int:
    ...
    print("=== LONG SHADOW LATEST-500 APPLY V1.0 ===")
