#!/usr/bin/env python3
"""
Diamond Trader Scanner Session Shadow Lab v1.0
...
verwachte-winst- en RR-filters worden NIET versoepeld.

Tijdbasis
---------
De virtuele entrytijd is detected_at in UTC. Dat is ook de opened_at die deze
shadow-simulatie gebruikt.
...
NO_12_17
    Exact dezelfde scanner-eligible signalen, behalve virtuele entries
    tussen 12:00:00 en 17:59:59 UTC.

ONLY_00_05
    Alleen scanner-eligible signalen met virtuele entry tussen
    00:00:00 en 05:59:59 UTC.
...
- schrijft uitsluitend eigen Session Shadow-bestanden in /var/data.

Gebruik
-------
    python3 scanner_session_shadow_lab.py --self-test
    python3 scanner_session_shadow_lab.py --update
    python3 scanner_session_shadow_lab.py --status
    python3 scanner_session_shadow_lab.py --update --no-print
"""
...
VERSION = "1.0"
MODE = "READ_ONLY_SCANNER_SESSION_SHADOW"
...
BASELINE_FILE = DATA_DIR / "diamond_scanner_session_shadow_baseline.json"
STATE_FILE = DATA_DIR / "diamond_scanner_session_shadow_state.json"
REPORT_FILE = DATA_DIR / "diamond_scanner_session_shadow_report.json"
TRADES_FILE = DATA_DIR / "diamond_scanner_session_shadow_trades.csv"

TARGET_CLOSED = 20
TIMEFRAME_MS = 15 * 60 * 1000
MAX_SIGNAL_KEYS = 30_000

VARIANTS = ("CURRENT", "NO_12_17", "ONLY_00_05")
...
def load_settings() -> Dict[str, Any]:
    ...
    try:
        ...
        result["stake_eur"] = max(
            5.0,
            to_float(
                scanner.get("stake_eur", risk.get("fixed_stake_quote", 120)),
                120,
            ),
        )
...
def entry_utc_hour(row: Dict[str, str]) -> Optional[int]:
    detected = parse_datetime(row.get("detected_at"))
    return detected.hour if detected is not None else None
...
def build_report(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "variants": summaries,
        "selection_rules": {
            "CURRENT": "alle huidige scanner shadow_eligible=True signalen",
            "NO_12_17": (
                "scanner-eligible EN detected_at UTC niet tussen 12:00 en 17:59"
            ),
            "ONLY_00_05": (
                "scanner-eligible EN detected_at UTC tussen 00:00 en 05:59"
            ),
        },
        "time_basis": "detected_at UTC = virtuele opened_at",
        "settings": state.get("settings") or {},
        "safety": SAFETY,
        "last_errors": state.get("last_errors") or [],
        "limitations": [
            "Prospectieve sessietest; geen live- of portefeuillesimulatie.",
            ...
            "De sessiehypothese is vooraf vastgelegd als 00-05 UTC sterk en 12-17 UTC zwak.",
        ],
    }
...
def run_update() -> Dict[str, Any]:
    ...
    if baseline_dt is None:
        raise ValueError("Ongeldige Session Shadow baseline")
...
def print_report(report: Dict[str, Any]) -> None:
    if not report:
        print("Session Shadow heeft nog geen rapport. Voer --update uit.")
        return

    print("=" * 82)
    print(" DIAMOND TRADER SCANNER SESSION SHADOW LAB")
    print("=" * 82)
    ...
    print(f"Tijdbasis              : {report.get('time_basis', '-')}")
    ...
    no_midday = variants.get("NO_12_17") or {}
    only_night = variants.get("ONLY_00_05") or {}

    print()
    print("VERSCHILLEN")
    print("-" * 82)
    print(
        f"NO_12_17 vs CURRENT : "
        f"delta_pnl=€{to_float(no_midday.get('net_pnl_eur')) - to_float(current.get('net_pnl_eur')):+.4f}"
    )
    print(
        f"ONLY_00_05 vs CURRENT: "
        f"delta_pnl=€{to_float(only_night.get('net_pnl_eur')) - to_float(current.get('net_pnl_eur')):+.4f}"
    )
...
def self_test() -> None:
    base = {
        "detected_at": "2026-08-08T02:01:00+00:00",
        "candle_timestamp": "2026-08-08T01:45:00+00:00",
        "symbol": "TEST/EUR",
    ...
    assert variant_accepts("NO_12_17", base)
    assert variant_accepts("ONLY_00_05", base)

    midday = {
        **base,
        "detected_at": "2026-08-08T14:01:00+00:00",
        "candle_timestamp": "2026-08-08T13:45:00+00:00",
    }
    assert variant_accepts("CURRENT", midday)
    assert not variant_accepts("NO_12_17", midday)
    assert not variant_accepts("ONLY_00_05", midday)

    evening = {
        **base,
        "detected_at": "2026-08-08T20:01:00+00:00",
        "candle_timestamp": "2026-08-08T19:45:00+00:00",
    }
    assert variant_accepts("CURRENT", evening)
    assert variant_accepts("NO_12_17", evening)
    assert not variant_accepts("ONLY_00_05", evening)

    rejected = {**base, "shadow_eligible": "False"}
    assert not variant_accepts("CURRENT", rejected)
    assert not variant_accepts("NO_12_17", rejected)
    assert not variant_accepts("ONLY_00_05", rejected)
    ...
    assert int(position["entry_utc_hour"]) == 2
    ...
    print("SCANNER_SESSION_SHADOW_SELF_TEST_OK")
    print("Varianten: CURRENT / NO_12_17 / ONLY_00_05")
    print("Tijdbasis: detected_at UTC")
    print("Orders mogelijk: NEE")
    print("Private API: NEE")
    print("Scannerfilters gewijzigd: NEE")
    print("Bot/config gewijzigd: NEE")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Diamond Trader Scanner Session Shadow Lab"
    )
