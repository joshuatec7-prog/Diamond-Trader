#!/usr/bin/env python3
"""
Diamond Trader - Strategy Comparison Report v1.2
...
REPORT_JSON = DATA / "diamond_strategy_comparison_report_v1_2.json"


RR_GUARDRAIL_EVIDENCE = {
    "source": "rr_080_120_shadow_analysis.py v1.0",
    "evaluated_at": "2026-08-07",
    "current_rr_min": 1.20,
    "recommendation": "KEEP_1_20",
    "reason": (
        "Virtuele historische RR-only kandidaten onder 1.20 bleven na "
        "kosten/spread duidelijk verliesgevend. Geen reden om de RR-grens "
        "nu te verlagen."
    ),
    "bands": {
        "RR_GE_1_10": {
...
def write_report():
    ...
    report = {
        ...
        "rr_guardrail": RR_GUARDRAIL_EVIDENCE,
    }
...
def main():
    ...
    print("============================================================")
    print(" DIAMOND TRADER - STRATEGY COMPARISON v1.2")
    print("============================================================")
    print()
    ...
    rr = report["rr_guardrail"]
    print("=== RR VEILIGHEIDSFILTER ===")
    print(f"Huidige minimum RR    : {rr['current_rr_min']:.2f}")
    for label in ("RR_GE_1_10", "RR_GE_1_00", "RR_GE_0_90", "RR_0_80_TO_1_20"):
        x = rr["bands"][label]
        print(
            f"{label:16} "
            f"{x['closed']:>3} gesloten | "
            f"{x['wins']}W/{x['losses']}L | "
            f"{x['winrate_pct']:.1f}% | "
            f"€{x['net_pnl_eur']:+.2f} | "
            f"PF {x['profit_factor']:.2f}"
        )
    print("Conclusie             : RR 1.20 behouden")
    print()
