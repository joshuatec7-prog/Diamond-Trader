...
VERSION = "2.0"
MODE = "READ_ONLY_LONG_COMBO_SHADOW_V2"
...
BASELINE_FILE = DATA_DIR / "diamond_long_combo_shadow_v2_baseline.json"
STATE_FILE = DATA_DIR / "diamond_long_combo_shadow_v2_state.json"
REPORT_FILE = DATA_DIR / "diamond_long_combo_shadow_v2_report.json"
TRADES_FILE = DATA_DIR / "diamond_long_combo_shadow_v2_trades.csv"
...
def build_report(
    ...
    return {
        ...
        "rules": {
            ...
            "WAIT15_100": (
                "entry +15m; exit-min €1.00; trail-lock €1.00"
            ),
            "WAIT15_050": (
                "entry +15m; exit-min €0.50; trail-lock €1.00"
            ),
...
def print_report(
    ...
    print(
        f"{'WAIT15_050':12s} vs WAIT15_100 "
...
def self_test() -> None:
    ...
    assert (
        VARIANTS["WAIT15_100"]["wait_minutes"]
        == 15
    )

    assert (
        VARIANTS["WAIT15_050"]["wait_minutes"]
        == 15
    )
    ...
    print(
        "WAIT15_100   : entry +15m   | exit €1.00 | trail-lock €1.00"
    )
    print(
        "WAIT15_050   : entry +15m   | exit €0.50 | trail-lock €1.00"
    )
    print(
...
def main() -> None:
    ...
    if args.no_print:
        ...
        print(
            ...
            f"WAIT15_100_closed="
            f"{(variants.get('WAIT15_100') or {}).get('closed', 0)} | "
            f"WAIT15_050_closed="
            f"{(variants.get('WAIT15_050') or {}).get('closed', 0)}"
        )
