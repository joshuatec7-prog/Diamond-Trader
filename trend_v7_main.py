...
logger = logging.getLogger('cryptobot_cleanroom_trend_v7')
TREND_MAX_OPEN_POSITIONS = 3
TREND_STOP_LOSS_PCT = 1.00
...
def build_trend_settings(primary_s: Settings) -> Settings:
    trend_s = replace(
        ...
        stop_loss_pct=TREND_STOP_LOSS_PCT,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description='CryptoBot Clean-Room Strategy B v7 - staged trend runner PAPER'
    )
    ...
    try:
        if args.status:
            print('=== STRATEGY B v7 | RANKED TREND | STOP 1.0% | LOCK €1 | STAGED RUNNER ===')
        ...
        logger.info(
            'gestart | STRATEGY B v7 STAGED RUNNER | PAPER ONLY | interval=%s | '
        ...
        while True:
            ...
            if ok == len(markets) and failed == 0:
                ...
                trend_db.set_data_health(
                    'READY', f'volledige ranked-trend-v7-staged-runner-cyclus ok={ok}'
                )
            elif ok > 0:
                consecutive_total_failures = 0
                trend_db.set_data_health(
                    'PARTIAL',
                    f'ranked-trend-v7-staged-runner-cyclus ok={ok} failed={failed}; {last_error}',
                )
            else:
                consecutive_total_failures += 1
                trend_db.set_data_health(
                    'DEGRADED',
                    last_error or f'alle {failed} trend-v7-marktcycli mislukt',
                )

            if consecutive_total_failures >= trend_s.max_consecutive_failed_cycles:
                logger.error(
                    'trend-v7-marktdata volledig onbereikbaar gedurende %s cycli',
