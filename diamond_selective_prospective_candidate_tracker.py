...
STATE = DATA / "diamond_selective_prospective_candidate_state.json"
REPORT = DATA / "diamond_selective_prospective_candidate_report.json"
...
def rules() -> List[Tuple[str, str, Callable[[Dict[str, Any]], bool]]]:
    return [
        (
            "CURRENT_ALL",
            "Ongewijzigde SELECTIVE referentie.",
            lambda row: True,
        ),
        (
            "GUARDED_MIX",
            "Alle LONG + SHORT alleen R/R 1.40-1.59.",
            lambda row: (
                row["side"] == "LONG"
                or (
                    row["side"] == "SHORT"
                    and rr_between(row, 1.40, 1.60)
                )
            ),
        ),
        (
            "RR_GE_140",
            "Alle SELECTIVE trades met R/R >=1.40.",
            lambda row: f(row.get("reward_risk")) >= 1.40,
        ),
        (
            "LONG_ALL",
            "Alle LONG SELECTIVE trades; diagnostische referentie.",
