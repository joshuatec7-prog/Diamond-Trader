...
logger = logging.getLogger('cryptobot_cleanroom_trend_v5')
...
def trend_db_path(primary_path: str) -> str:
    ...
    return str(p.with_name(f'{stem}_trend_v5{suffix}'))
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description='CryptoBot Clean-Room Strategy B v5 - trend runner PAPER'
    )
    ...
    try:
        if args.status:
            print('=== STRATEGY B v5 | RANKED TREND | RUNNER | GEEN HARDE TP ===')
        ...
        logger.info(
            'gestart | STRATEGY B v5 RUNNER | PAPER ONLY | interval=%s | universe=%s | '
        ...
        while True:
            ...
            if ok == len(markets) and failed == 0:
                ...
                trend_db.set_data_health(
                    'READY', f'volledige ranked-trend-v5-runner-cyclus ok={ok}'
                )
            elif ok > 0:
                consecutive_total_failures = 0
                trend_db.set_data_health(
                    'PARTIAL',
                    f'ranked-trend-v5-runner-cyclus ok={ok} failed={failed}; {last_error}',
                )
            else:
                consecutive_total_failures += 1
                trend_db.set_data_health(
                    'DEGRADED',
                    last_error or f'alle {failed} trend-v5-marktcycli mislukt',
                )

            if consecutive_total_failures >= trend_s.max_consecutive_failed_cycles:
                logger.error(
                    'trend-v5-marktdata volledig onbereikbaar gedurende %s cycli',
