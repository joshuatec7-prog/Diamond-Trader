...
logger = logging.getLogger('cryptobot_cleanroom_continuation_v6')
CONTINUATION_MAX_OPEN_POSITIONS = 3
CONTINUATION_STOP_LOSS_PCT = 1.00
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description='CryptoBot Clean-Room Strategy C v6 - staged continuation runner PAPER'
    )
    ...
    try:
        if args.status:
            print('=== STRATEGY C v6 | PULLBACK CONTINUATION | STOP 1.0% | LOCK €1 | STAGED RUNNER ===')
        ...
        logger.info(
            'gestart | STRATEGY C v6 STAGED RUNNER | PAPER ONLY | interval=%s | '
        ...
        while True:
            ...
            if ok == len(markets) and failed == 0:
                ...
                continuation_db.set_data_health(
                    'READY', f'volledige continuation-v6-staged-runner-cyclus ok={ok}'
                )
            elif ok > 0:
                consecutive_total_failures = 0
                continuation_db.set_data_health(
                    'PARTIAL',
                    f'continuation-v6-staged-runner-cyclus ok={ok} failed={failed}; {last_error}',
                )
            else:
                consecutive_total_failures += 1
                continuation_db.set_data_health(
                    'DEGRADED',
                    last_error or f'alle {failed} continuation-v6-marktcycli mislukt',
                )

            if consecutive_total_failures >= continuation_s.max_consecutive_failed_cycles:
                logger.error(
                    'continuation-v6-marktdata volledig onbereikbaar gedurende %s cycli',
