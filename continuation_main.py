...
logger = logging.getLogger('cryptobot_cleanroom_continuation')
STOP = False
...
def shared_universe(
    ...
    while not STOP:
        ...
        if markets:
            existing = continuation_db.universe()
            if existing and existing != markets:
                raise RuntimeError('continuation-universe wijkt af van primaire vaste universe')
            if not existing:
                continuation_db.set_universe(markets)
            continuation_db.set_data_health(
                'UNIVERSE_READY',
                f'gedeelde universe beschikbaar: {len(markets)} markten',
            )
            return markets

        continuation_db.set_data_health('STARTING', 'wacht op primaire vaste universe')
        if once:
            return None
        for _ in range(5):
            if STOP:
...
def scan_market_for_candidate(
    ...
    if len(candles) < strategy.required_candles():
        raise RuntimeError(f'{market}: onvoldoende gesloten candles voor continuation-strategie')
    ...
    if not math.isfinite(score):
        decision = Decision('SKIP', 'ongeldige_continuation_score', decision.metrics)
        db.save_decision(market, latest.timestamp_ms, decision)
        logger.info('%s SKIP | %s', market, decision.reason)
        return None

    return ContinuationCandidate(market, latest.timestamp_ms, decision, score)
...
def execute_ranked_candidates(
    ...
    for rank, candidate in enumerate(ranked, start=1):
        ...
        try:
            ...
            db.save_decision(candidate.market, candidate.candle_ts, decision)
            logger.exception('%s: orderboek voor continuation-entry mislukt', candidate.market)
            continue
...
def run_continuation_cycle(
    ...
    for market in markets:
        if STOP:
            break
        try:
            candidate = scan_market_for_candidate(
                market,
                api,
                db,
                strategy,
            ...
            logger.exception('%s: continuation-scan mislukt', market)

    if candidates and not STOP:
        entry_failures = execute_ranked_candidates(
            candidates,
            api,
            db,
            trader,
        ...
        if entry_failures:
            ...
            last_error = (
                f'{entry_failures} continuation entry-orderboek request(s) mislukt'
            )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description='CryptoBot Clean-Room Strategy C - trend continuation/pullback PAPER'
    )
    ...
    try:
        if args.status:
            print('=== STRATEGY C v1 | TREND PULLBACK CONTINUATION ===')
        ...
        markets = shared_universe(primary_s, continuation_db, args.once)
        if markets is None:
        ...
        logger.info(
            'gestart | STRATEGY C CONTINUATION | PAPER ONLY | interval=%s | '
            'universe=%s | max_open=%s | db=%s',
        ...
        while True:
            ok, failed, last_error = run_continuation_cycle(
                api,
                continuation_db,
                strategy,
                trader,
            ...
            if ok == len(markets) and failed == 0:
                ...
                continuation_db.set_data_health(
                    'READY',
                    f'volledige continuation-cyclus ok={ok}',
                )
            elif ok > 0:
                consecutive_total_failures = 0
                continuation_db.set_data_health(
                    'PARTIAL',
                    f'continuation-cyclus ok={ok} failed={failed}; {last_error}',
                )
            else:
                consecutive_total_failures += 1
                continuation_db.set_data_health(
                    'DEGRADED',
                    last_error or f'alle {failed} continuation-marktcycli mislukt',
                )

            if consecutive_total_failures >= continuation_s.max_consecutive_failed_cycles:
                logger.error(
                    'continuation-marktdata volledig onbereikbaar gedurende %s cycli',
