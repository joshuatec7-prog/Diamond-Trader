...
VERSION = "POST_COVERAGE_1.0"

DATA = Path(os.getenv("DIAMOND_DATA_DIR", "/var/data"))
SOURCE = DATA / "diamond_scanner_selective_shadow_trades.csv"
STATE = DATA / "diamond_selective_post_coverage_state.json"
REPORT = DATA / "diamond_selective_post_coverage_report.json"
...
def rules():
    return [
        ("CURRENT_ALL","Ongewijzigde SELECTIVE referentie.",lambda r: True),
        ("RR_GE_160","R/R minimaal 1.60.",
         lambda r: f(r.get("reward_risk")) >= 1.60),
        ("TREND_BREAKOUT","Alleen trend_breakout.",
         lambda r: str(r.get("strategy") or "") == "trend_breakout"),
    ]
