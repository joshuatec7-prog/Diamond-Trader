#!/usr/bin/env python3
"""
Diamond Trader - Strategy Comparison Report v1.0
...
REPORT_JSON = DATA / "diamond_strategy_comparison_report_v1_0.json"
...
def paper_short_v3():
    ...
    closes = [r for r in tx if str(r.get("side") or r.get("action") or "").upper() == "SHORT_CLOSE"]

    # diamond_transactions.csv kan een naamloze/andere header hebben; fallback via ruwe regels.
    if not closes:
        try:
            with TRANSACTIONS.open("r", encoding="utf-8") as f:
                raw = [line.strip().split(",") for line in f if ",SHORT_CLOSE," in line]
            # bekende kolommen uit Diamond Trader transactiebestand
            # timestamp,symbol,action,price,amount,quote,fee,spread,pnl,minutes,reason,opened_by_bot
            closes = []
            for a in raw:
                if len(a) >= 12:
                    closes.append({
    ...
    pnls = [fnum(r.get("net_pnl_eur") or r.get("pnl_quote") or r.get("pnl")) for r in test_closes]
    wins = sum(1 for x in pnls if x > 0)
...
def main():
    ...
    print("============================================================")
    print(" DIAMOND TRADER - STRATEGY COMPARISON v1.0")
    print("============================================================")
    print()
