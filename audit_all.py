...
STRATEGY_D = 'D_ADAPTIVE_TREND_V1'
...
def _update_source(settings: Settings, strategy: str, source_path: str) -> dict[str, int | str]:
    ...
    try:
        ...
        if decision_sig != old_decision_sig or max_candle_ts != old_max_candle:
            with audit:
                ...
                updated += _update_pending(source, audit, strategy, settings.interval, interval_ms)
                _state_set(audit, f'decision_sig:{strategy}', decision_sig)
                _state_set(audit, f'max_candle:{strategy}', max_candle_ts)
    finally:
        source.close()
        audit.close()
    return {'sources': 1, 'imported': imported, 'updated': updated, 'db_path': out_path}
...
def update_all_missed_trade_audit(settings: Settings) -> dict[str, int | str]:
    ...
    results = [
        ...
        _update_source(settings, STRATEGY_D, adaptive_db_path(settings.db_path)),
    ]
    return {
...
def print_all_audit_report(settings: Settings) -> None:
    ...
    conn = _connect(path)
    try:
        _print_strategy_report(conn, STRATEGY_A, 'STRATEGIE A | MEAN REVERSION SKIPS', cost_floor)
        _print_strategy_report(
            conn, STRATEGY_B,
        ...
        _print_strategy_report(
            conn, STRATEGY_D,
            'STRATEGIE D v1 | ADAPTIVE TREND FOLLOWER | ATR RUNNER SKIPS',
            cost_floor,
        )
    finally:
        conn.close()
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description='Clean-Room missed-trade audit voor actuele PAPER strategie A/B/C/D'
    )
    ...
    logger.info(
        'gestart | READ-ONLY PAPER MISSED-TRADE AUDIT A/Bv7/Cv6/Dv1 | interval=%s',
        settings.interval,
    )
    while True:
        try:
            ...
            if int(result['imported']) or int(result['updated']):
                logger.info(
                    ...
                    result['sources'], result['imported'], result['updated'], result['db_path'],
                )
        except Exception:
